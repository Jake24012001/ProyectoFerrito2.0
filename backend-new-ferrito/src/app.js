...
// Rutas privadas de ejecucion o llamada
...
// 🌍 Servir frontend Angular (⚠️ Usar index.html si no es SSR)
const frontendPath = path.join(__dirname, '../../paginawebFerrito/dist/paginaweb-ferrito/browser');
app.use(express.static(frontendPath));

// ✅ Solo después de rutas API
app.get('*', (req, res, next) => {
  if (req.originalUrl.startsWith('/api/')) {
    return next(); // evita capturar rutas API
  }
  res.sendFile(path.join(frontendPath, 'index.html'));
});

// ❌ Ruta no encontrada (solo se activará si no hay match previo)
