...
async function registrarCarrito({ usuario_id, fecha_creacion, estado }) {
  if (!usuario_id) {
    throw new Error("No se puede crear carrito sin usuario");
  }
...
async function cerrarCarrito(id_carrito) {
  const { rows } = await pool.query(
    `UPDATE carrito SET estado = 'C'
     WHERE id_carrito = $1 RETURNING *`,
    [id_carrito]
  );
  return rows[0];
}
...
async function obtenerOCrearCarritoPorEmail(email) {
  ...
  return await registrarCarrito({
    usuario_id: usuarioRes.rows[0].id_usuario,
